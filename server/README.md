...
- **Local:** `JWT_SECRET` is mandatory (minimum 32 chars) and validated at startup; optional `CORS_ORIGINS` (or `CORS_ALLOW_VERCEL=1` to allow any `https://*.vercel.app`), `AZURE_STORAGE_CONNECTION_STRING`
- **Database provider:** set `DB_PROVIDER=sqlite` (default) or `DB_PROVIDER=sqlserver`.
- **SQLite mode:** `./library.db` locally; `/tmp/library.db` on Azure App Service.
- **SQL Server mode:** set `SQL_SERVER`, `SQL_DATABASE`, `SQL_USER`, `SQL_PASSWORD`, `SQL_ENCRYPT` (typically `true` for Azure SQL). The API initializes required tables (`users`, `books`, `borrow_records`) on startup.
